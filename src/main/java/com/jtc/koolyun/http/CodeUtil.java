...
public class CodeUtil {
	 ...
	 */
	public static byte[] pin2PinBlock(String aPin) throws Exception {
		int tTemp = 1;

		int tPinLen = aPin.length();
		if (tPinLen > 12 || tPinLen < 4) {
			throw new Exception("PIN格式错误");
		}
		...
		try {
			...
			throw new Exception("PIN转PINBLOCK失败", e);
		}
		return tByte;
	}
	 ...
	 * @throws Exception
	 */
	private static byte[] formatPan(String aPan) throws Exception {
	...
	public static String md5(String string) {
		...
		return CodeUtil.byte2HexString(hash);
	}
}
